( function () {
	...
	var number = require( "../../../common/standardization/number" );
	var street = require( "../../../common/standardization/street" );
	var numberDirection = require( "../../../common/standardization/numberDirection" );
	var unitType = require( "../../../common/standardization/unitType" );
		...
		function processAddress() {
			...
			function parseAddressLines() {
				...
				function getBestLocationString() {
					...
					function replaceLocationMarker( oldMarker, newMarker ) {
						var property;
						var newProperty;
						var regExp = new RegExp( oldMarker, "g" );

						for ( property in locationsObj ) {
							if ( locationsObj.hasOwnProperty( property ) ) {
								newProperty = property.replace( regExp, newMarker );
								delete locationsObj[ property ];
								locationsObj[ newProperty ] = calculateLocationScore( newProperty );
							}
						}
					}
					...
					regExp = /^(R*P*H*,?u{0,2},?)(?:(?:(p?)([DNx]+?(?:t?N+)?)(t?f?)(&)(p?)([DNx]+?(?:t?N+)?)(t?f?))|(?:(N*p?)([DNx]+?(?:t?N+)?)(t?N?f?)))??(?:(,?u{0,2},?P*,?))(x*)(,?[sS]*,?Z*,?C*)$/;
					removeLocationsWithoutMatch( regExp );
					logStatus( "getBestLocationString", "valid locationsObj", locationsObj );

					//	Remove multiple start starts,
					regExp = /^.*S.*S.*$/;
					removeLocationsWithMatch( regExp );
					replaceLocationMarker( "s", "S" );
					if ( Object.keys( locationsObj ).length === 1 ) {
						return;
					}
					...
					for ( phrase = 0; phrase < numPhrases; phrase++ ) {
						...
						for ( wordStart = 0; wordStart < numWords; wordStart++ ) {
							for ( wordEnd = numWords - 1; wordEnd >= wordStart; wordEnd-- ) {
								...
								if ( highwayContract.match( wordString ) ) {
									for ( word = wordStart; word <= wordEnd; word++ ) {
										addMarker( "H" );
										logStatus( "parseAddressLines", "highway match", wordString );
									}
								}
								if ( poBox.match( wordString ) ) {
									for ( word = wordStart; word <= wordEnd; word++ ) {
										addMarker( "P" );
										logStatus( "parseAddressLines", "poBox match", wordString );
									}
								}
								if ( state( wordString ) ) {
									for ( word = wordStart; word <= wordEnd; word++ ) {
										addMarker( word === wordStart ? "S" : "s" );
								...
								if ( numberDirection.match( wordString ) ) {
									for ( word = wordStart; word <= wordEnd; word++ ) {
										addMarker( "D" );
										logStatus( "parseAddressLines", "number direction match", wordString );
									}
								}
				...
				function replaceUnknowns() {
					var matchesRegExp = /^(R*P*H*,?u{0,2},?)(?:(?:(p?)([DNx]+?(?:t?N+)?)(t?f?)(&)(p?)([DNx]+?(?:t?N+)?)(t?f?))|(?:(N*p?)([DNx]+?(?:t?N+)?)(t?N?f?)))??(?:(,?u{0,2},?P*,?))(x*)(,?[sS]*,?Z*,?C*)$/;
					var matches;
					var numx, i;
					var xReplacement;
					var numberDirectionRegExp = /^D+$/;
					...
					if ( matchesRegExp.test( locationValue ) ) {
						...
						if ( matches[ 3 ] ) {
							...
							if ( numberDirectionRegExp.test( matches[ 3 ] ) ) {
								locationValue += matches[ 3 ];
							}
							else {
							...
							if ( numberDirectionRegExp.test( matches[ 7 ] ) ) {
								locationValue += matches[ 7 ];
							}
							else {
							...
							if ( matches[ 10 ] ) {
								...
								if ( numberDirectionRegExp.test( matches[ 10 ] ) ) {
									locationValue += matches[ 10 ];
								}
								else {
					...
					locationValue = locationValue.replace( /DD/g, "Dd" );
					locationValue = locationValue.replace( /nD/g, "nd" );
					locationValue = locationValue.replace( /D&D/g, "d&d" );
						...
						if ( locationValue[ i ] !== locationValue[ i + 1 ] ) {
							...
							switch ( type ) {
								...
								case "R":
									...
									break;
								case "H":
									addressObject.highway_contract = highwayContract.match( phrase );
									logStatus( "generateAddressObject", "'" + phrase + "' = HC: " + addressObject.highway_contract );
									break;
								case "P":
									addressObject.po_box = poBox.match( phrase );
									logStatus( "generateAddressObject", "'" + phrase + "' = POBox: " + addressObject.po_box );
									break;
								case "D":
									numberDirectionObj = numberDirection.match( phrase );
									addressObject.number = numberDirectionObj.number;
									addressObject.prefix = numberDirectionObj.direction;
									logStatus( "generateAddressObject", "'" + phrase + "' = Number: " + addressObject.number );
									logStatus( "generateAddressObject", "'" + phrase + "' = Prefix: " + addressObject.prefix );
									break;
								case "d":
									if ( twoStreets ) {
										numberDirectionObj = numberDirection.match( phrase );
										addressObject.street2 = numberDirectionObj.number;
										addressObject.suffix2 = numberDirectionObj.direction;
										logStatus( "generateAddressObject", "'" + phrase + "' = Street2: " + addressObject.street2 );
										logStatus( "generateAddressObject", "'" + phrase + "' = Suffix2: " + addressObject.suffix2 );
									}
									else {
										numberDirectionObj = numberDirection.match( phrase );
										addressObject.street = numberDirectionObj.number;
										addressObject.suffix = numberDirectionObj.direction;
										logStatus( "generateAddressObject", "'" + phrase + "' = Street: " + addressObject.street );
										logStatus( "generateAddressObject", "'" + phrase + "' = Suffix: " + addressObject.suffix );
									}
									break;
