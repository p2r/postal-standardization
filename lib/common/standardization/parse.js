		...
		function processAddress() {
			...
			function parseAddressLines() {
				...
				function generateAddressObject() {
					...
					function getStreetParts( phrase ) {
						var streetsRegExp = /^(.*)(?:\&|and)(.*)$/i;
						var streets;

						if ( streetsRegExp.test( phrase ) ) {
							streets = phrase.match( streetsRegExp );
							addressObject.street1 = clean( streets[ 1 ] );
							addressObject.street2 = clean( streets[ 2 ] );
						}
						else {
					...
					while ( i < numMarkers ) {
						...
						if ( locationValue[ i ] !== locationValue[ i + 1 ] ) {
							...
							switch ( type ) {
								...
								case "s":
									addressObject.street = phrase;
									getStreetParts( phrase );
									if ( addressObject.street1 ) {
										logStatus( "generateAddressObject", "'" + phrase + "' = Street1: " + addressObject.street1 );
